...
const SOURCE_META = {
  ...
  klart: { label: 'Klart', color: 'var(--series-klart)' },
...
function sampleAt(source, t) {
  ...
  return {
    ...
    dir: a.dir,
    // Nederbörd anges som intensitet (mm/h) och gäller hela intervallet framåt.
    rate: a.precipRate ?? null,
...
function sourceLabel(s) {
  const base = (SOURCE_META[s.id] || {}).label || s.name;
  return s.substituteFor === 'klart' ? `${base} (i stället för Klart)` : base;
}
...
function renderNow() {
  ...
  $('nowDesc').innerHTML = `${SYMBOL_TEXT[c.symbol] || 'Prognos'}<small>${c.n} av 3 källor · spridning ${fmtNum(c.spread)} °C</small>`;

  const m = [
    ['Nederbörd', `${fmtNum(c.rate, 1)} mm/h`],
    ['Regnrisk', `${Math.round(c.wetShare * 100)} %`],
...
function renderSources() {
  ...
  const third = activeSources().find((s) => s.id === 'klart' || s.id === 'ecmwf');
  $('thirdCredit').textContent = third && third.id === 'ecmwf'
    ? 'ECMWF via Open-Meteo (CC BY 4.0)' : 'Klart.se';
...
function renderChart() {
  ...
  rows.forEach((row, i) => {
    ...
    parts.push(`<rect class="bar" x="${(x(i) - bw / 2).toFixed(1)}" y="${(pBot - h).toFixed(1)}" width="${bw.toFixed(1)}" height="${h.toFixed(1)}" rx="2" opacity="${(0.45 + 0.55 * row.c.wetShare).toFixed(2)}"/>`);
  });
...
async function refresh() {
  ...
  try {
    ...
    if (!data.sources || !data.sources.length) throw new Error('Ingen källa svarade');
    state.data = data;
...
function useGeolocation() {
  ...
  navigator.geolocation.getCurrentPosition(async (pos) => {
    ...
    try { info = await (await fetch(`/api/revgeo?lat=${lat}&lon=${lon}`)).json(); } catch { /* behåll fallback */ }
...
function initSearch() {
  ...
  input.addEventListener('input', () => {
    ...
    timer = setTimeout(async () => {
      try {
        const { results } = await (await fetch(`/api/geo?q=${encodeURIComponent(q)}`)).json();
